document.querySelectorAll('.pips').forEach(el => {
    ...
    const searchIndex = [...document.querySelectorAll('main section[id]:not(.hero)')].flatMap(section => {
      const sectionTitle = section.querySelector(':scope > .section-head h2')?.textContent || 'Kompendium';
      const entries = [...section.querySelectorAll('h3')].map(heading => {
        const container = heading.closest('article,details,.region') || heading.parentElement;
        return { title:heading.textContent.trim(), section:sectionTitle, href:`#${section.id}`, text:container.textContent.replace(/\s+/g,' ').trim() };
      });
      return [{title:sectionTitle, section:'Bereich', href:`#${section.id}`, text:section.querySelector('.section-intro')?.textContent || sectionTitle}, ...entries];
    });
    const renderSearch = () => {
      const query = normalize(globalSearch.value.trim());
      if (query.length < 2) { searchResults.innerHTML = '<p>Gib mindestens zwei Zeichen ein. Durchsucht werden Klassen, Übel, Welt, Wissen und Links.</p>'; return; }
      const terms = query.split(/\s+/).filter(Boolean);
      const matches = searchIndex.map(item => {
        const haystack = normalize(`${item.title} ${item.section} ${item.text}`);
        const score = terms.reduce((sum, term) => sum + (normalize(item.title).includes(term) ? 4 : 0) + (haystack.includes(term) ? 1 : 0), 0);
        return {...item, score};
      }).filter(item => item.score >= terms.length).sort((a,b) => b.score - a.score || a.title.localeCompare(b.title,'de')).slice(0,20);
      searchResults.innerHTML = matches.length ? matches.map(item => `<a class="search-result" href="${item.href}"><strong>${item.title}</strong><span>${item.text.slice(0,170)}${item.text.length > 170 ? '…' : ''}</span><small>${item.section}</small></a>`).join('') : '<p>Keine passenden Einträge gefunden.</p>';
      searchResults.querySelectorAll('a').forEach(link => link.addEventListener('click', () => searchDialog.close()));
    };
    document.querySelector('.search-toggle').addEventListener('click', () => { searchDialog.showModal(); requestAnimationFrame(() => globalSearch.focus()); });
